...
function config = applyPriorityTables(sensors, constants)
    [BusGen1 BusGen2] = selectGenerators(sensors, constants); %Del1,Del2 in Mehdi's code
    %[Shedding1 Shedding2] = selectShedding(sensors, constants, BusGen1, BusGen2) %C1, C2 in Mehdi's code
...
function [Shedding1 Shedding2] = selectShedding(sensors, constants, BusGen1, BusGen2) 
    ...
    while (pwrReqGen1 > constants.generatorOutput(1))
        ...
        if (gen(0) == 1 && pwrReqGen1 > constants.generatorOutput(1)) % now do it for the left side if still over
    ...
    while (pwrReqGen2 > constants.generatorOutput(2))
        ...
        if (gen(1) == 2 && pwrReqGen2 > constants.generatorOutput(2)) % now do it for the right side if still over
    ...
    while (pwrReqApu > constants.generatorOutput(3))
        ...
        if (gen(1) == 3 && pwrReqGen2 > constants.generatorOutput(3)) % now do it for the right side if still over
